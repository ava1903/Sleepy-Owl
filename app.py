...
<style>
  /* ══ RESET & BASE ══════════════════════════════════════════════════ */
  .stApp {{ background-color: {PRIMARY}; color: #E8F4FD; }}
  ...
  /* Radio buttons — fix the red/orange active dot */
  [data-testid="stSidebar"] [data-baseweb="radio"] label {{
    color: #C9D8E8 !important;
    font-size: 0.9rem;
    padding: 4px 0;
  }}
  /* The radio circle — unselected */
  [data-testid="stSidebar"] [data-baseweb="radio"] [role="radio"] {{
    border-color: {ACCENT} !important;
    background: transparent !important;
  }}
  /* The radio circle — selected fill */
  [data-testid="stSidebar"] [data-baseweb="radio"] [role="radio"][aria-checked="true"] {{
    border-color: {TEAL} !important;
    background-color: {TEAL} !important;
  }}
  /* Inner dot of selected radio */
  [data-testid="stSidebar"] [data-baseweb="radio"] [role="radio"][aria-checked="true"]::after {{
    background-color: #071828 !important;
  }}

  /* ══ SLIDERS — fix red/orange thumb & track ════════════════════════ */
  /* Track (filled portion) */
  [data-testid="stSlider"] [data-testid="stThumbValue"] {{ color: {HIGHLIGHT} !important; font-weight: 700; }}
  div[data-baseweb="slider"] [data-testid="stSlider"] div {{ background: {ACCENT} !important; }}
  /* Thumb circle */
  [data-baseweb="slider"] div[role="slider"] {{
    background-color: {TEAL} !important;
    border-color: {HIGHLIGHT} !important;
    box-shadow: 0 0 0 4px rgba(46,204,113,0.20) !important;
  }}
  /* Filled track */
  [data-baseweb="slider"] div[data-testid="stSlider"] > div > div > div:first-child {{
    background: linear-gradient(90deg, {ACCENT}, {TEAL}) !important;
  }}
  /* Generic override for all slider internal divs */
  div[class*="StyledSliderTrack"] {{ background: {NAVY2} !important; }}
  div[class*="StyledSliderInnerTrack"] {{ background: {TEAL} !important; }}
  div[class*="StyledSliderThumb"] {{
    background: {TEAL} !important;
    border: 2px solid {HIGHLIGHT} !important;
  }}
  /* Slider label text */
  [data-testid="stSlider"] label {{ color: #A8C8E8 !important; font-size: 0.85rem !important; }}
  /* Slider value bubble */
  ...
  [data-baseweb="select"] * {{ color: #E8F4FD !important; background-color: {CARD_BG} !important; }}
  [data-baseweb="menu"] {{ background-color: #0F2A40 !important; border: 1px solid {ACCENT} !important; }}
  [data-baseweb="option"]:hover {{ background-color: {NAVY2} !important; }}
  /* Multiselect tags */
  [data-baseweb="tag"] {{
  ...
  /* ══ TABS ════════════════════════════════════════════════════════════ */
  [data-baseweb="tab-list"] {{ background-color: {CARD_BG} !important; border-radius: 8px 8px 0 0; gap: 2px; }}
  ...
  button[data-baseweb="tab"]:hover {{ color: #C9D8E8 !important; background: {NAVY2} !important; }}
  ...
  [data-baseweb="tab-panel"] {{ background-color: {PRIMARY} !important; padding-top: 1rem !important; }}

  /* ══ DATAFRAMES ═════════════════════════════════════════════════════ */
  [data-testid="stDataFrame"] {{ background: {CARD_BG}; border-radius: 8px; overflow: hidden; }}
  [data-testid="stDataFrame"] th {{ background: {NAVY2} !important; color: {HIGHLIGHT} !important; font-size: 0.8rem; }}
  [data-testid="stDataFrame"] td {{ color: #C9D8E8 !important; font-size: 0.82rem; }}
  ...
  .insight-card strong {{ color: #E8F4FD; }}
  ...
  .action-card strong {{ color: #E8F4FD; }}
  ...
  .dataset-card b {{ color: {HIGHLIGHT}; }}
  ...
  .pred-yes   {{ background:#071A0F; border:2px solid {HIGHLIGHT}; border-radius:10px; padding:1.4rem; text-align:center; }}
  .pred-maybe {{ background:#1A1200; border:2px solid {AMBER};     border-radius:10px; padding:1.4rem; text-align:center; }}
  .pred-no    {{ background:#1A0707; border:2px solid {RED};       border-radius:10px; padding:1.4rem; text-align:center; }}
  .pred-label {{ font-size:2.2rem; font-weight:800; }}
  .pred-sub   {{ font-size:0.83rem; color:#7AAAC5; margin-top:0.4rem; }}
  ...
  /* ══ MISC ════════════════════════════════════════════════════════════ */
  hr {{ border-color: {NAVY2}; margin: 1rem 0; opacity: 0.5; }}
  [data-testid="stSidebar"] hr {{ border-color: {ACCENT}44; }}
  p, li {{ color: #C9D8E8; line-height: 1.6; }}
  .stAlert {{ background: {CARD_BG} !important; border-color: {ACCENT} !important; color: #C9D8E8 !important; }}
...
def dark_fig(fig, height=400):
    fig.update_layout(
        ...
        font=dict(color="#C9D8E8", size=12),
        height=height,
...
def page_clustering(df):
    ...
    with col4:
        ...
        fig.update_layout(
            ...
            scene=dict(
                xaxis=dict(title="PC1", backgroundcolor=PRIMARY),
                yaxis=dict(title="PC2", backgroundcolor=PRIMARY),
                zaxis=dict(title="PC3", backgroundcolor=PRIMARY),
            ),
