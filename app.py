...
def page_regression(df):
    ...
    # ── Extra: Spend by income bracket × categories heatmap ───────────
    st.markdown("<br>", unsafe_allow_html=True)
    section_header("Spend Drivers Deep-Dive")
    col_r1, col_r2 = st.columns(2)

    with col_r1:
        # Scatter: income vs spend, coloured by n_categories
        fig = px.scatter(
            df.dropna(subset=['q4_income_numeric','q14_monthly_spend_inr']),
            x='q4_income_numeric', y='q14_monthly_spend_inr',
            color='n_categories_purchased',
            color_continuous_scale=[[0,ACCENT],[0.5,TEAL],[1,HIGHLIGHT]],
            opacity=0.5, size_max=7,
            labels={'q4_income_numeric':'Income Level','q14_monthly_spend_inr':'Monthly Spend (₹)',
                    'n_categories_purchased':'Categories'},
            title='Income vs Spend (colour = categories purchased)',
        )
        dark_fig(fig, 380, cartesian=False)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with col_r2:
        ...
        fig = go.Figure()
        palette = [ACCENT, TEAL, HIGHLIGHT, AMBER, RED]
        for i, t in enumerate(tenure_order):
            sub = df[df['q9_tenure'] == t]['q14_monthly_spend_inr'].dropna()
            if len(sub) == 0: continue
            fig.add_trace(go.Box(
                y=sub, name=t, marker_color=palette[i],
                boxmean=True, line_width=1.5,
            ))
        fig.update_layout(
            title='Monthly Spend by Customer Tenure',
    ...
    # Model comparison: LR vs RF metrics side by side
    section_header("Model Comparison — Linear Regression vs Random Forest")
    lr_r2   = r2_score(yte, yp_lr)
    lr_rmse = np.sqrt(mean_squared_error(yte, yp_lr))
    lr_mae  = mean_absolute_error(yte, yp_lr)

    comp_data = {
        'Metric': ['R²','RMSE (₹)','MAE (₹)'],
        'Linear Regression': [round(lr_r2,3), round(lr_rmse,0), round(lr_mae,0)],
        'Random Forest':     [round(r2,3),    round(rmse,0),    round(mae,0)],
    }
    comp_df_reg = pd.DataFrame(comp_data)
    st.dataframe(comp_df_reg, use_container_width=True, hide_index=True)

    fig = go.Figure()
    fig.add_trace(go.Bar(name='Linear Regression', x=comp_df_reg['Metric'],
                         y=comp_df_reg['Linear Regression'], marker_color=ACCENT))
    fig.add_trace(go.Bar(name='Random Forest',     x=comp_df_reg['Metric'],
                         y=comp_df_reg['Random Forest'],     marker_color=TEAL))
    fig.update_layout(barmode='group', title='LR vs RF — Key Metrics',
