...
def page_regression(df):
    ...
    model_map = {
        "lr":    ("Linear Regression",  res["yp_lr"],    res["metrics_lr"],    res["coefs_lr"]),
        "ridge": ("Ridge (α=10)",       res["yp_ridge"], res["metrics_ridge"], res["coefs_ridge"]),
        "lasso": ("Lasso (α=5)",        res["yp_lasso"], res["metrics_lasso"], res["coefs_lasso"]),
    }

    for tab, mkey in zip([tab_lr, tab_ridge, tab_lasso], ["lr", "ridge", "lasso"]):
        with tab:
            mname, yp, mets, coefs = model_map[mkey]
            st.markdown(f"*Model: **{mname}***")
            c1,c2,c3,c4 = st.columns(4)
            c1.markdown(metric_card("R² Score", str(mets["r2"]),           mname),               unsafe_allow_html=True)
            c2.markdown(metric_card("RMSE",     f"₹{mets['rmse']:,}",   "Root Mean Sq Error"), unsafe_allow_html=True)
            c3.markdown(metric_card("MAE",      f"₹{mets['mae']:,}",    "Mean Abs Error"),      unsafe_allow_html=True)
            c4.markdown(metric_card("MSE",      f"₹{mets['mse']:,}",    "Mean Sq Error"),       unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            col1, col2 = st.columns(2)
            with col1:
            ...
            with col2:
                ...
                dark_fig(fig, 360)
                st.plotly_chart(fig, use_container_width=True)
            section_header(f"Coefficients — {mname}")
            coefs_s = coefs.sort_values()
            show_c = coefs_s[coefs_s.abs() > 0.001] if mkey == "lasso" else coefs_s
            fig = go.Figure(go.Bar(
                x=show_c.values, y=[REG_LABELS.get(f,f) for f in show_c.index],
                orientation="h",
                marker=dict(color=[ACCENT if v >= 0 else RED for v in show_c.values]),
                text=[f"{v:+.3f}" for v in show_c.values], textposition="outside",
            ))
            fig.update_layout(xaxis_title="Coefficient (Standardised)", yaxis_title="")
            dark_fig(fig, 420)
            st.plotly_chart(fig, use_container_width=True)
            if mkey == "lasso":
                n_zero = (coefs.abs() < 0.001).sum()
                st.markdown(insight_card(
                    f"<strong>Lasso shrinkage:</strong> {n_zero} of {len(coefs)} features "
                    "driven to exactly zero — automatic feature selection."
                ), unsafe_allow_html=True)
    ...
    with tab_rf:
        ...
        c1,c2,c3,c4 = st.columns(4)
        c1.markdown(metric_card("R² Score", str(mets["r2"]),         "Random Forest"),       unsafe_allow_html=True)
        c2.markdown(metric_card("RMSE",     f"₹{mets['rmse']:,}", "Root Mean Sq Error"),   unsafe_allow_html=True)
        c3.markdown(metric_card("MAE",      f"₹{mets['mae']:,}",  "Mean Abs Error"),        unsafe_allow_html=True)
        c4.markdown(metric_card("MSE",      f"₹{mets['mse']:,}",  "Mean Sq Error"),         unsafe_allow_html=True)
        st.markdown("<br>", unsafe_allow_html=True)
        ...
        fig = go.Figure(go.Bar(
            x=fi.values, y=[REG_LABELS.get(f,f) for f in fi.index], orientation="h",
        ...
        comp_rows = [
            ...
            {"Model":"Ridge (α=10)",       **res["metrics_ridge"]},
            {"Model":"Lasso (α=5)",         **res["metrics_lasso"]},
            {"Model":"Random Forest",        **res["metrics_rf"]},
        ]
        comp_df = pd.DataFrame(comp_rows)
        st.dataframe(
            comp_df.style.highlight_max(subset=["r2"], color="#D4EDDA")
                         .highlight_min(subset=["rmse","mae","mse"], color="#D4EDDA"),
            use_container_width=True, hide_index=True
        )
        fig = go.Figure()
        for metric, color, label in [("r2",ACCENT,"R²"),("rmse",TEAL,"RMSE"),("mae",AMBER,"MAE")]:
        ...
        fig.update_layout(barmode="group", title="Model Comparison", yaxis_title="Score / Error")
        dark_fig(fig, 360)
        st.plotly_chart(fig, use_container_width=True)

    with tab_sim:
        section_header("Spend Prediction Simulator")
        st.markdown("*Adjust the customer profile below — all four models predict simultaneously.*")
        left, right = st.columns([1, 1])
        with left:
            sim_vals = {}
            for feat in REG_FEATURES:
                col_data = df[feat].dropna()
                min_v, max_v, mean_v = float(col_data.min()), float(col_data.max()), float(col_data.mean())
                label = REG_LABELS.get(feat, feat)
                if max_v - min_v > 1:
                    sim_vals[feat] = st.slider(label, min_v, max_v, mean_v,
                                               step=0.5 if max_v <= 10 else 50.0, key=f"rs_{feat}")
                else:
                    sim_vals[feat] = st.selectbox(label, [0,1], index=int(round(mean_v)),
                                                  format_func=lambda x: "Yes" if x else "No",
                                                  key=f"rss_{feat}")
        with right:
            input_df_sim  = pd.DataFrame([[sim_vals[f] for f in REG_FEATURES]], columns=REG_FEATURES)
            input_scaled  = res["sc"].transform(input_df_sim)
            pred_rf    = float(res["rf"].predict(input_df_sim)[0])
            pred_lr    = float(res["lr"].predict(input_scaled)[0])
            pred_ridge = float(res["ridge"].predict(input_scaled)[0])
            pred_lasso = float(res["lasso"].predict(input_scaled)[0])
            pc1,pc2 = st.columns(2)
            pc3,pc4 = st.columns(2)
            pc1.markdown(metric_card("Linear Reg",    f"₹{pred_lr:,.0f}",    "Prediction"), unsafe_allow_html=True)
            pc2.markdown(metric_card("Ridge (α=10)",  f"₹{pred_ridge:,.0f}", "Prediction"), unsafe_allow_html=True)
            pc3.markdown(metric_card("Lasso (α=5)",   f"₹{pred_lasso:,.0f}", "Prediction"), unsafe_allow_html=True)
            pc4.markdown(metric_card("Random Forest", f"₹{pred_rf:,.0f}",    "Best Fit"),   unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            baseline = float(res["rf"].predict(
                pd.DataFrame([[float(res["Xtr"][f].mean()) for f in REG_FEATURES]], columns=REG_FEATURES)
            )[0])
            contribs = []
            for feat in REG_FEATURES:
                row = {f: float(res["Xtr"][f].mean()) for f in REG_FEATURES}
                row[feat] = sim_vals[feat]
            ...
            fig_wf = go.Figure(go.Waterfall(
                ...
                text=[f"₹{c[1]:+.0f}" for c in top_c] + [f"₹{total_d:+.0f}"],
                textposition="outside",
                decreasing=dict(marker=dict(color=RED)),
                increasing=dict(marker=dict(color=HIGHLIGHT)),
                totals=dict(marker=dict(color=ACCENT)),
            ))
            fig_wf.update_layout(title="RF Spend Drivers vs Average Customer", xaxis_title="₹ Impact")
            dark_fig(fig_wf, 420)
