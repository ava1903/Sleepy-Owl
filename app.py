...
<style>
  ...
  section[data-testid="stSidebar"] {{ background-color: #0A1929; }}
  ...
  .pred-yes  {{ background:#0D2D1E; border:2px solid {HIGHLIGHT}; border-radius:10px; padding:1.2rem; text-align:center; }}
  .pred-maybe{{ background:#2D2000; border:2px solid {AMBER};     border-radius:10px; padding:1.2rem; text-align:center; }}
  .pred-no   {{ background:#2D0D0D; border:2px solid {RED};       border-radius:10px; padding:1.2rem; text-align:center; }}
  .pred-label{{ font-size:2rem; font-weight:800; }}
  .pred-sub  {{ font-size:0.82rem; color:#8BAFC8; margin-top:0.3rem; }}
  ...
  /* ── tab tweaks ── */
  button[data-baseweb="tab"] {{ color: #8BAFC8 !important; }}
  button[data-baseweb="tab"][aria-selected="true"] {{ color: {HIGHLIGHT} !important; border-bottom-color: {HIGHLIGHT} !important; }}

  /* ── divider ── */
  hr {{ border-color: {NAVY2}; margin: 1rem 0; }}
</style>
